...
svy_dt %>%
  tbl_svysummary(include =c('nt_ch_stunt','nt_ch_wast',  'nt_ch_any_anem', "combined_indicator"), by="Year", percent = "column",
...
svy_dt %>%
  tbl_svysummary(include =c("agemonths", "hc27", "hv025", "shecoreg", "education", "hv024", "hv270", "hfs", "minimumDV"), by="Year", 
                 digits = list(all_categorical()~c(0,1))) |> 
  bold_labels() |> 
...
ch1<-conc_chart(dt1, dt2, dt3, chart_of = "Stunting", tag="A")
ch1
...
ch2<-conc_chart(dt1, dt2, dt3, chart_of = "Wasting",tag="B")
ch2
...
ch3<-conc_chart(dt1, dt2, dt3, chart_of = "Anemia", tag="C")
ch3
...
ch4<-conc_chart(dt1, dt2, dt3, chart_of = "Co-occurence", tag="D")
ch4
