...
Packages <- c("sjmisc","sf","ggspatial", "expss","naniar","gtsummary","foreign","survey",'labelled',"readxl", "tidyverse", "haven","rockchalk", "forcats", "INLA", "data.table", "srvyr")
...
est.fun1 <- function(data, variable, by) {
  ...
  for (i in 1:length(categories)) {
    o = categories[i]
    data$v <- data |> select(all_of(variable)) |> deframe()
...
tab.fun1 <- function(dataset, multinomlist=NA, by){
  ...
  tab1 <- svydt |> 
    tbl_svysummary(by = by,
                   ...
                   statistic = list(all_categorical() ~ c("{n}/{N},<br>{p}"))) |> 
    modify_table_body(~.x |>
